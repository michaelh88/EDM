...
final class Controller_Import extends Controller_Website {
    ...
    public function action_import() {
		...
		// $this->template->jslib[] = 'jquery.min';
		// $this->template->jslib[] = 'jquery-ui-1.8.22.min';
	...
	public function action_ajax_plupload() {
		...
		$directory_temp = DOCROOT.'misc/temp/';
		$dire = 'temp0';
		$directory_temp .=$dire . "/";
		$targetDir = $directory_temp;
		...
		if ($out) {
			...
			die('{"jsonrpc" : "2.0",  "message": "Failed to open output stream.", "id" : "id"}');
		...
		if (!$chunks || $chunk == $chunks - 1) {
			...
			$nom_zip_temporaire = "";
			$nombre_documents = 0;
			$this->nom_fichier_zip_import = $fileName;
			
			$dire = strtolower(Text::random('alnum', 20));
			$this->extract_zip($filePath, $dire);
			if ($this->error_msg != "") {
				die('{"jsonrpc" : "2.0",  "message": "'.$this->error_msg.'", "id" : "id"}');
			}
			...
			if (!($this->is_file_duplicate())) {
				...
				if ($nombre_documents > 0) {
					...
					$new_lot->nom_zip_original = $fileName;
					$new_lot->commande_vivetic = 'Aucune';
					...
					rename($directory_down."/".$dire.".zip", $directory_down."/".$this->nom_lot.".zip");
					
					$action_utilisateur = "Création du lot " . $inserted_lot->lot_id;
					$loginfo = "[Utilisateur : " . $this->current_user->username . "] [Action : " . $action_utilisateur . "]";
					Kohana::$log->add(LOG::INFO, $loginfo);
					
					
				} else {
					$sms = "<span class='upload-fail'>Fichier <b>" . $fileName . "</b> :</span><br/><span class='upload-message'>Aucune image trouvée dans le lot.</span>";
					die('{"jsonrpc" : "2.0",  "message": "'.$sms.'", "id" : "id"}');
				}
			} else {
				$doublon = sizeof($this->tb_duplicated_file);
				$sms = "<span class='upload-fail'>Fichier <b>" . $fileName . "</b> :</span><br/><span class='upload-message'>Nom de fichier identique trouvé dans le fichier importé, merci de corriger.</span>";
				die('{"jsonrpc" : "2.0",  "message": "'.$sms.'", "id" : "id"}');
			}
			$ret = "<span class='upload-success'>Fichier <b>". $this->nom_zip_import ."</b> :</span><br/><span class='upload-message'> Importation terminée avec succès, nouveau lot crée :<b>". $this->nom_lot ."</b> dont ". sizeof($this->tb_document) ." fichier(s) exploitable(s) sur ". sizeof($this->tb_file_list) ."</span>";
			die('{"jsonrpc" : "2.0",  "message": "'.$ret.'", "id" : "id"}');
			
			
		}
		// Return Success JSON-RPC response
		die('{"jsonrpc" : "2.0", "result" : "complet","message":"'.$fileName.'", "id" : "id"}');
	}
    ...
    private function do_convert($image, $directory_plati) {
		...
		$command = "convert -quality 60 " . $image . " " . $directory_plati;
        $ret_com = exec($command, $output, $return_var);
        if ($return_var != 0) {
			die('{"jsonrpc" : "2.0",  "message": "Une erreur est survenue lors de la conversion de l\'image en PDF", "id" : "id"}');
	...
	public function action_ajax_update_plupload() {
		...
		if (!$chunks || $chunk == $chunks - 1) {
			...
			if ($nom_temp == "") {
				$sms = "<span class='upload-fail'>Fichier <b>" . $fileName . "</b> :</span><br/><span class='upload-message'>Le lot choisi n'est pas un lot valide</span>";
				$this->error_msg = $sms;
				die('{"jsonrpc" : "2.0", "message" : "'.$this->error_msg.'", "id" : "id"}');
			}
			...
			if (isset($_FILES['file'])) {
				...
				if (!($this->is_file_duplicate())) {
                    ...
                    if ($nombre_documents > 0) {
                        ...
                        $new_lot->commande_vivetic = 'Aucune';
                        $new_lot->nom_zip_temp = $nom_zip_temporaire;
						$new_lot->modificateur_lot = $this->current_user->id;

                        $new_lot->nombre_doc = $nombre_documents;
                        $new_lot->save();
						$sql_update = "UPDATE lot SET datetime_modification=NOW() WHERE lot_id = " . $new_lot->lot_id;
						DB::query(Database::UPDATE, $sql_update)->execute();
						
                        $this->nom_lot = $new_lot->nom_zip;
                        $this->nom_zip_import = $fileName;
						$inserted_lot = ORM::factory('Lot', $new_lot->lot_id);
						
						rename($directory_down."/".$dire.".zip", $directory_down."/".$this->nom_lot.".zip");
						
						$action_utilisateur = "Modification du lot " . $inserted_lot->lot_id;
						$loginfo = "[Utilisateur : " . $this->current_user->username . "] [Action : " . $action_utilisateur . "]";
						Kohana::$log->add(LOG::INFO, $loginfo);
						
                    } else {
                        $sms = "<hr style='margin-bottom: 5px;'/><span class='upload-fail'>Fichier <b>" . $fileName . "</b> :</span><br/><span class='upload-message'>Aucune image trouvée dans le lot</span>";
                        $this->error_msg = $sms;
						die('{"jsonrpc" : "2.0", "message" : "'.$this->error_msg.'", "id" : "id"}');
                    }
                } else {
                    $doublon = sizeof($this->tb_duplicated_file);
                    $sms = "<hr style='margin-bottom: 5px;'/><span class='upload-fail'>Fichier <b>" . $fileName . "</b> :</span><br/><span class='upload-message'>Noms de fichiers identiques trouvés dans le fichier importé, merci de corriger.</span>";
                    $this->error_msg = $sms;
					die('{"jsonrpc" : "2.0", "message" : "'.$this->error_msg.'", "id" : "id"}');
                }
				$ret = "<span class='upload-success'>Fichier <b>" . $this->nom_zip_import . "</b> :</span><br/><span class='upload-message'> Importation terminée avec succès, nouveau lot crée :<b>" . $this->nom_lot . "</b> dont " . sizeof($this->tb_document) . " fichier(s) exploitable(s) sur " . sizeof($this->tb_file_list) . "</span>";
		...
		// Return Success JSON-RPC response
		die('{"jsonrpc" : "2.0", "message" : "'.$fileName.'", "id" : "id"}');
	}
        ...
        try {
            ...
            if ($file = Upload::save($the_file, NULL, $directory_temp)) {
                ...
                if (!($this->is_file_duplicate())) {
                    ...
                    $doublon = sizeof($this->tb_duplicated_file);
                    $sms = '<hr style="margin-bottom: 5px;"/><span class="upload-fail">Fichier <b>' . $the_file["name"] . '</b> :</span><br/><span class="upload-message">Noms de fichiers identiques trouvés dans le fichier importé, merci de corriger.</span>';
                    $this->error_msg = $sms;
                }
                $this->tb_result = array(
                    "doublon" => $doublon,
